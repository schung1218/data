...
static struct dma_async_tx_descriptor *ma35d1_prep_dma_memcpy(struct dma_chan
							      *chan,
							      dma_addr_t dst,
							      dma_addr_t src,
							      size_t len,
...
static struct dma_async_tx_descriptor *ma35d1_prep_slave_sg(struct dma_chan
							    *chan, struct scatterlist
							    *sgl,
							    unsigned int sg_len,
							    enum
							    ...
							    void *context)
{
	...
	for_each_sg(sgl, sgent, sg_len, i) {
		d->sg[i].ctl =
		    (ch->runtime_ctrl |
		     PDMA_TXCNT((sg_dma_len(sgent) / ch->runtime_width) -
