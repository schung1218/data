...
#define PDMA_TXCNT_Pos 16DMA_CYCLIC
...
static void ma35d1_dma_advance_work(struct ma35d1_dma_chan *edmac)
{
	...
	if (!list_empty(&edmac->active) || list_empty(&edmac->queue)) {
		spin_unlock_irqrestore(&edmac->lock, flags);
		printk("ma35d1_dma_advance_workv NULL\n");
		return;
	}
	...
	spin_unlock_irqrestore(&edmac->lock, flags);
	printk("ma35d1_dma_advance_workv LEAVE\n");
	LEAVE();
}
...
static irqreturn_t ma35d1_dma_interrupt(int irq, void *dev_id)
{
	...
	unsigned int pdma_int_status =
	    ma35d1_pdma_edmac_read(&edma->channels[0], PDMA_INTSTS);
	unsigned int pdma_status = 
		ma35d1_pdma_edmac_read(&edma->channels[0], PDMA_TDSTS);
	...
	for (i = (edma->num_channels - 1); i >= 0; i--) {
		...
		if (pdma_int_status & (1 << (edma->channels[i].ch + 8))) {
			...
			ma35d1_dma_edmac_interrupt(&edma->channels[i],
				INTERRUPT_TIMEOUT);
			value = (ma35d1_pdma_edmac_read(&edma->channels[i],
				PDMA_TOUTEN) & ~(1 << (edma->channels[i].ch)));
			ma35d1_pdma_edmac_write(&edma->channels[i],
				PDMA_TOUTEN, value);
			ma35d1_pdma_edmac_write(&edma->channels[i],
				PDMA_INTSTS,(1 <<(edma->channels[i].ch + 8)));
		}
	}
...
static int ma35d1_dma_alloc_chan_resources(struct dma_chan *chan)
{
	...
	for (i = 0; i < MA35D1_DMA_MAX_CHAN_DESCRIPTORS; i++) {
		struct ma35d1_dma_desc *desc;
...
static void ma35d1_dma_free_chan_resources(struct dma_chan *chan)
{
	...
	spin_lock_irqsave(&edmac->lock, flags);
	edmac->edma->hw_shutdown(edmac);linux/linux.mk
	...
	unsigned long flags)
{
	...
	for (offset = 0; offset < len; offset += bytes) {
		...
		bytes = min_t(size_t, len - offset, MA35D1_DMA_MAX_CHAN_BYTES);
	...
	void *context)
{
	...
	for_each_sg(sgl, sg, sg_len, i) {
		...
		if (len > MA35D1_DMA_MAX_CHAN_BYTES) {
		...
		desc->size = len;
		desc->config.reqsel = *(u32 *)edmac->slave_config.peripheral_config;
		if (data != NULL) {
	...
	unsigned long flags)
{
	...
	if (period_len > MA35D1_DMA_MAX_CHAN_BYTES) {
...
static int ma35d1_dma_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < pdata->num_channels; i++) {
		...
		edmac->edma = edma;
		#ifdef EN_HW_SG
		edmac->sg_addr = (u32)dma_map_single(edmac->dev,
			(void *)(edmac->sg_base), SG_LEN, DMA_BIDIRECTIONAL);
		#endif
	...
	dma_dev->src_addr_widths = BIT(DMA_SLAVE_BUSWIDTH_4_BYTES) | BIT(DMA_SLAVE_BUSWIDTH_2_BYTES) |
		BIT(DMA_SLAVE_BUSWIDTH_1_BYTE);
	dma_dev->dst_addr_widths = BIT(DMA_SLAVE_BUSWIDTH_4_BYTES) | BIT(DMA_SLAVE_BUSWIDTH_2_BYTES) |
		BIT(DMA_SLAVE_BUSWIDTH_1_BYTE);
	
	dma_set_max_seg_size(dma_dev->dev, MA35D1_DMA_MAX_CHAN_BYTES);
