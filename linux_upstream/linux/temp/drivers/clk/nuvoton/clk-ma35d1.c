...
static void ma35d1_clocks_unLock_reg(struct regmap *regmap)
{
	int ret;
...
static int ma35d1_clocks_probe(struct platform_device *pdev)
{
	...
	regmap = syscon_regmap_lookup_by_compatible("nuvoton,ma35d1-sys");
	if (IS_ERR(regmap)) {
	...
	/* Unlock clock registers */	
	ma35d1_clocks_unLock_reg(regmap);
...
MODULE_AUTHOR("SCHung <schung@nuvoton.com>, CFLi <CFLi0@nuvoton.com>");
